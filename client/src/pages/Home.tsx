...
function Button({ children, onClick, disabled = false, secondary = false }: { children: React.ReactNode; onClick?: () => void; disabled?: boolean; secondary?: boolean }) {
  ...
  const handleClick = () => { setPressed(true); onClick?.(); };
  return <button disabled={disabled} onClick={handleClick} className={`group w-full rounded-2xl px-5 py-4 text-base font-bold transition-all active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-40 ${secondary ? "border border-white/70 bg-white text-[#242229] hover:bg-white/90" : pressed ? "bg-[#7c3aed] text-white shadow-[0_0_28px_rgba(124,58,237,.25)]" : "bg-white text-[#242229] shadow-[0_0_18px_rgba(255,255,255,.12)] hover:bg-white/90"}`}>{children}</button>;
}
...
function SelectStep({ title, options, value, setValue, next, progress }: { title: string; options: [string, string][]; value?: string; setValue: (v: string) => void; next: () => void; progress: number }) {
  ...
  useEffect(() => { setSelectedId(undefined); }, [title]);
  const grid = options.length >= 28 ? "grid-cols-7" : options.length >= 10 ? "grid-cols-3" : options.length === 4 ? "grid-cols-1" : "grid-cols-1";
  return <Shell progress={progress} onBack={() => window.dispatchEvent(new Event('quiz:back'))}><div className="flex flex-1 flex-col items-center justify-center"><h2 className="flex max-w-[560px] items-center justify-center gap-8 text-center text-[30px] font-extrabold leading-[1.08] tracking-[-.025em] text-white sm:text-[30px]"><span className="motion-safe:animate-[pulse_2.4s_ease-in-out_infinite] grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#7c3aed] text-[16px]">↓</span>{title}<span className="motion-safe:animate-[pulse_2.4s_ease-in-out_infinite] grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#7c3aed] text-[16px]">↓</span></h2><div className={`mt-8 grid w-full max-w-[560px] gap-3 ${grid}`}>{options.map(([id, label]) => <button key={id} onClick={() => { setSelectedId(id); setValue(id); window.setTimeout(next, 220); }} className={`min-h-[56px] rounded-xl border-2 px-2 text-center text-[17px] font-extrabold transition-all active:scale-[.98] ${selectedId === id ? "border-[#7c3aed] bg-[#7c3aed] text-[#171426] shadow-[0_0_22px_rgba(124,58,237,.38)]" : "border-white bg-white text-[#242229] hover:-translate-y-0.5 hover:bg-white/90"}`}>{label}</button>)}</div><button onClick={() => window.dispatchEvent(new Event('quiz:back'))} className="mt-8 rounded-xl border-2 border-white bg-white px-6 py-3 text-base font-bold text-[#242229] shadow-[0_0_18px_rgba(255,255,255,.12)] transition active:scale-[.98] hover:bg-white/90 active:border-[#7c3aed] active:bg-[#7c3aed] active:text-white">← Voltar</button></div></Shell>;
}
